...
class Guo2021SQuery(UnstructuredDustMap):
    """
    ...
    def __init__(self, map_fname=None):
        ...
        if map_fname is None:
            map_fname = os.path.join(data_dir(), 'guo2021', 'guo2021.fits')
        f=fits.open(map_fname)
        self._dists = np.arange(0.2,6.2,0.2)
        self._l = f[1].data['L']
        self._b = f[1].data['B']
        self._A = f[1].data['EBV']
        self._sigma_A = f[1].data['ERR_EBV']
        ...
        super(Guo2021SQuery, self).__init__(coords, max_pix_scale, metric_p=2)
...
class Guo2021AQuery(UnstructuredDustMap):
    """
    ...
    def __init__(self, map_fname=None):
        ...
        """
        if map_fname is None:
            map_fname = os.path.join(data_dir(), 'guo2021', 'allsky2021.fits')
        f=fits.open(map_fname)
        self._dists = np.arange(0.2,6.2,0.2)
        self._l = f[1].data['L']
        self._b = f[1].data['B']
        ...
        super(Guo2021AQuery, self).__init__(coords, max_pix_scale, metric_p=2)
...
def fetch(clobber=False):
    ...
    archive_fname = os.path.join(dest_dir, 'guo2021.zip')
    archive_md5sum = '738f1d017ffb52e7707f52e023baa8af'
