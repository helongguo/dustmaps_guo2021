...
#
# chen2014.py
# Reads the Guo et al. (2021) southern sky 3D dust map, based on the multi-band photometry of SMSS, Gaia DR2, 2MASS and WISE and parallaxes from Gaia DR2.
#
...
class Chen2013VVVQuery(UnstructuredDustMap):
    """
    The southern sky 3D dust map of Guo et al. (2021), based on the multi-band photometry of SMSS, Gaia DR2, 2MASS and WISE and parallaxes from Gaia DR2.
    For details on how to use this map, see the original paper:
    https://ui.adsabs.harvard.edu/abs/2021ApJ...906...47G/abstract.
    """

    def __init__(self, map_fname=None,color='Hks'):
        """
        Args:
        ...
        if map_fname is None:
            map_fname = os.path.join(data_dir(), 'chen2013', 'chen2013VVV.fits')
        f=fits.open(map_fname)
        self._dists = np.arange(1.0,11.0,1.0)
        self._l = f[1].data['L']
        self._b = f[1].data['B']
        self._A = f[1].data['E'+color]
        self._A =self._A.reshape(len(self._A),10)
        self._sigma_A = f[1].data['e_E'+color]
        self._sigma_A =self._sigma_A.reshape(len(self._sigma_A),10)
        ...
        super(Chen2013VVVQuery, self).__init__(coords, max_pix_scale, metric_p=2)
...
class Chen2013GLIMPSEQuery(UnstructuredDustMap):
    """
    The file 'allsky2021.fits' is an all-sky extinction maps, which combine our 3D extinction maps in the southern sky and those from the literature.
    For details on how to use this map, see the original paper:
    https://ui.adsabs.harvard.edu/abs/2021ApJ...906...47G/abstract.
    """

    def __init__(self, map_fname=None,color='k3.6'):
        """
        Args:
            map_fname (Optional[:obj:`str`]): Filename at which the map is stored.
            Defaults to ``None``, meaning that the default filename is used.
        """
        if map_fname is None:
            map_fname = os.path.join(data_dir(), 'chen2013', 'chen2013GLIMPSE.fits')
        f=fits.open(map_fname)
        self._dists = np.arange(1.0,11.0,1.0)
        self._l = f[1].data['L']
        self._b = f[1].data['B']
        ...
        super(Chen2013GLIMPSEQuery, self).__init__(coords, max_pix_scale, metric_p=2)

    @ensure_flat_galactic
    def query(self, coords, return_sigma=False):
        """
        Returns E(B-V) at the given coordinates. 
...
def fetch(clobber=False):
    """
    Downloads the 3D dust map of Guo et al. (2021).
    Args:
    ...
    url = 'http://paperdata.china-vo.org/guo/dust/guo2021.zip'
    archive_fname = os.path.join(dest_dir, 'guo2019.zip')
    ...
    with zipfile.ZipFile(archive_fname, 'r') as f:
        f.extract('guo2021.fits', path=dest_dir)
        f.extract('allsky2021.fits', path=dest_dir)
        f.extract('readme.txt', path=dest_dir)
    os.rename(os.path.join(dest_dir, 'guo2021.fits'), table_fname)
    os.rename(os.path.join(dest_dir, 'allsky2021.fits'), table_fname)
