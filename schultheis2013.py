...
class Schultheis2013Query(UnstructuredDustMap):
    """
    ...
    bulge covering -10<l<10 and -10<b<5. For details on how to use this map, see the original paper:
    https://ui.adsabs.harvard.edu/abs/2014A%26A...566A.120S/abstract.
    """

    def __init__(self, map_fname=None,color='Jk'):
        """
        Args:
        ...
        if map_fname is None:
            map_fname = os.path.join(data_dir(), 'schultheis', 'schultheis2013.fits')
        f=fits.open(map_fname)
        self._dists = np.arange(0.0,10.5,0.5)
        self._l = f[1].data['L']
        self._b = f[1].data['B']
        self._A = f[1].data['E'+color]
        self._A =self._A.reshape(len(self._A),21)
        self._sigma_A = f[1].data['e_E'+color]
        self._sigma_A =self._sigma_A.reshape(len(self._sigma_A),21)
        ...
        super(Schultheis2013Query, self).__init__(coords, max_pix_scale, metric_p=2)

    @ensure_flat_galactic
    def query(self, coords, return_sigma=False):
        """
        Returns E(J-K) at the given coordinates. Can also
...
def fetch(clobber=False):
    """
    Downloads the 3D dust map of schultheis et al. (2013).
    Args:
        clobber (Optional[bool]): If ``True``, any existing file will be
            ...
            corruption.
    """
    dest_dir = fname_pattern = os.path.join(data_dir(), 'schultheis2013')
    table_fname = os.path.join(dest_dir, 'schultheis2013.fits')
    
    
    # Check if the FITS table already exists
    table_md5sum = 'b9d6906ac138eba984ddb59397477aee'
    ...
    url = 'http://paperdata.china-vo.org/guo/dust/schultheis2013.zip'
    archive_fname = os.path.join(dest_dir, 'schultheis2013.zip')
    archive_md5sum = '8ef82fea1e508931de24910abf03c093'
